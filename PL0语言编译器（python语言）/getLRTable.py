...
class LRTableGenerator:
    ...
    def getClosure(self,itemGroup, allItems):
    ...
    def findNextItem(self,LRItem, allItem):
    ...
    def buildStatusGroup(self,startLRGroup, allItem):
        ...
        for status in statusGroup:
            ...
            for item in thisGroup:
                ...
                if item.dotPos < len(right):
                    ...
                    nextWord = right[item.dotPos]
                    try:
                        wordList[nextWord].append(thisGroup.index(item))
                    except KeyError:
                        wordList[nextWord] = []
                        wordList[nextWord].append(thisGroup.index(item))
    ...
    # 根据所有状态，来构建LR分析表
    def buildLRTable(self,statusGroup, VN, VT, allRules):
        LRTable = {}  # 用字典存储LR分析表
        ...
        for status in statusGroup:
            ...
            for word, shiftStatus in shiftMap.items():
                # GOTO表：接收非终结符迁移状态
                if word in VN:
                    LRTable[(statusNO, word)] = ("goto", shiftStatus.groupID)
                # ACTION表中的shift(移进)部分
                elif word in VT:
                    LRTable[(statusNO, word)] = ("shift", shiftStatus.groupID)


        # 状态1 遇到 # 则语法分析成功结束
        LRTable[(1,'#')] = ("accept", "accept")
        return LRTable

    def printLRTable(self):
        table = self.LRTable
        for K, V in table.items():
            statusNO, shiftWord = K
            action, shiftStatus = V
            if action != "reduce":
                shiftStatusNO = shiftStatus
